    ...
    if(trigger.isUpdate){
         for(Quote q :trigger.New){
             if(q.Status =='Revision' &&q.Status  !=trigger.oldMap.get(q.Id).Status){
                 q.Version_Number__c =q.Version_Number__c ==null?1 :(q.Version_Number__c+1);
                     q.Status='Draft';
                   if (q.Version_Number__c != null && q.Version_Number__c > 1) {
                 string oldversionNumber ='-RV'+trigger.oldMap.get(q.Id).Version_Number__c;
                   string newversionNumber ='-RV'+  q.Version_Number__c;
                 q.Name = q.Name.replace(oldversionNumber,newversionNumber);
                   }else{
                       q.Name= q.Name+'-RV'+  q.Version_Number__c; 
                   }
             }
        } 
    }
